...
2. Create the vault file with your MCP API key:
   ```
   cp group_vars/karteikarten/vault.yml.example group_vars/karteikarten/vault.yml
   # edit mcp_api_key to a real random value
   ansible-vault encrypt group_vars/karteikarten/vault.yml
   ```
3. In `../docker-compose.yml`, replace the placeholder `VIRTUAL_HOST` with
   the real hostname you want nginx-proxy/Cloudflare to route.
...
After the *first* push to `main`, the image is published to
`ghcr.io/dennis-h1/cards` but starts out **private** by GHCR default -- go
to the package's GitHub settings and set visibility to Public once
(matches the "public" choice made for this repo), otherwise both the VPS
pull and any manual `docker pull` will fail with 401/403.
